 ...
 * removing the disabled class from the options, 
 * removing the correct or incorrect classes,
 * disabling the check answer button and making it reappear. 
...
function readyQuestion() {
  ...
  checkButton.classList.remove("checkactive");
...
/**
 * newQuestion function will first set the image element to image src from the questions array.
 ...
 * If the last question has been reached, the function will display the submit button and hide the next button.
 * It will call the select answer function.
 * It will increment the n counter by one to advance the question.
 */
 ...
 * If they are not the same, the incorrect class is adding turning the option red. A message is displayed to tell the user that they were incorrect, and tells the correct answer. 
 ...
 * If lower than 7, it suggests the user to the lesson page.
 */
function submit() {
  ...
  if (numCorrect > 7) {
    quizContainer.innerHTML = `<div class="end-game p-5 mb-5">You scored ${numCorrect} / ${QUESTIONS.length}. Well done! Tap start to try the game again.</div>`;
    backButtons.classList.remove("hidden");
    lessonButton.classList.add("hidden");
  } else {
